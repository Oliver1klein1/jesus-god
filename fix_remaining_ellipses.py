#!/usr/bin/env python3
"""
Final comprehensive fix for corrupted ellipses in XHTML files
"""
import os
import re
from pathlib import Path

def fix_remaining_ellipses(file_path):
    """Fix remaining corrupted ellipses in a single file"""
    print(f"Fixing remaining ellipses in {file_path}...")
    ...
    # More aggressive patterns for remaining cases
    patterns = [
        # Fix any remaining ellipses in class names (multiple passes)
        (r'class="([^"]*?)\.\.\.([^"]*?)"', r'class="\1-\2"'),
        (r'class="([^"]*?)\.\.\.([^"]*?)\.\.\.([^"]*?)"', r'class="\1-\2-\3"'),
        (r'class="([^"]*?)\.\.\.([^"]*?)\.\.\.([^"]*?)\.\.\.([^"]*?)"', r'class="\1-\2-\3-\4"'),
        
        # Fix ellipses in CSS properties and values
        ...
        # Fix ellipses in text content (be more aggressive)
        (r'([A-Za-z]+)\.\.\.([A-Za-z]+)', r'\1-\2'),
        ...
        (r'([A-Za-z]+)\.\.\.([A-Za-z]+)\.\.\.([A-Za-z]+)', r'\1-\2-\3'),
        (r'([A-Za-z]+)\.\.\.([A-Za-z]+)\.\.\.([A-Za-z]+)\.\.\.([A-Za-z]+)', r'\1-\2-\3-\4'),
    ]
    
    # Apply regex patterns multiple times to catch nested cases
    for _ in range(3):  # Multiple passes
        for pattern, replacement in patterns:
            content = re.sub(pattern, replacement, content)
    
    # Additional specific replacements for remaining cases
    specific_replacements = [
        ...
        ('witness...card', 'witness-card'),
        ('witness...header', 'witness-header'),
        ('witness...number', 'witness-number'),
...
def main():
    """Fix remaining ellipses in all XHTML files"""
    ...
    for file_path in xhtml_files:
        fixed, remaining = fix_remaining_ellipses(file_path)
        total_fixed += fixed
    ...
    if total_remaining > 0:
        ...
        for file_path in xhtml_files:
            ...
            if count > 0:
                ...
                # Show a few examples of remaining ellipses
                lines = content.split('\n')
                examples = []
                for line in lines:
                    if '...' in line and len(examples) < 3:
                        examples.append(line.strip())
                if examples:
                    print(f"    Examples: {examples[0][:80]}...")
    else:
